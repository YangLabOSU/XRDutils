...
def getdata(fname, sr=43):
    ...
    if sr == 43:
        ...
        material = fname
    data = pd.read_csv(path+fname, skiprows=sr,
    ...
    for i in range(len(fnames)):
        ...
        if sctype == 'xrd':
            ...
            if annotatepeaks:
                ...
                # print(peaks)
                for peak in peaks:
                    plt.annotate(str(2*d.Position[peak]+zeroshift-pkshift), [
                                 2*d.Position[peak]+zeroshift-pkshift, d.Count[peak]+30], fontsize=7)

            plt.title(title)
            plt.semilogy(2*d.Position+zeroshift-pkshift,
                         d.Count, lw=1, label=title)
...
def main(argv):
    helpmsg = 'XRDutils_getdata -i <inputfolder>'
    inputfolder = ''
    try:
